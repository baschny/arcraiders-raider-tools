...
- `SNAPSHOT_ALLOWED_EMAIL` — defaults to `dev@localhost`; use that email in the dev bearer token to exercise tutorial snapshots. For deployments, create `infra/.env.cdk` from `infra/.env.cdk.example`; its value overrides `infra/.env` during CDK synthesis and is applied to the production Lambdas. Local snapshot payloads are stored in DynamoDB Local rather than S3.
