...
- A tiny Node HTTP server that hosts the **same** `profile.ts` / `state.ts` / `links.ts` / `arctracker-user-proxy.ts` Lambda handlers used in production, pointed at the local DynamoDB where applicable.
...
- `LOCAL_TOKEN_ENCRYPTION_KEY` — local-only key material used to encrypt linked ArcTracker tokens in DynamoDB Local when `KMS_KEY_ID` is not set.
### 3. Vite dev server
```bash
# Repo root
